...
def main():
    ...
    request = urllib2.Request(urlparse.urljoin(conf['ckan_of_worms.site_url'], 'api/1/datasets/weights'),
        headers = headers)
    response = urllib2.urlopen(request)
    global weight_by_dataset_name
    weight_by_dataset_name = conv.check(conv.pipe(
...
#                ),
    ...
    for name, endpoint, topic, message in fedmsg.tail_messages(**fedmsg_config):
        ...
        if kind == 'dataset':
            dataset = message['msg']
            name = dataset['name']
            existing_weight = weight_by_dataset_name.get(name)
            ...
            if weight != existing_weight:
                log.info(u'Updating states after "{} dataset {}"'.format(action, dataset['name']))
                if weight is None:
                    del weight_by_dataset_name[name]
                else:
                    weight_by_dataset_name[name] = weight
                send_states()
        else:
...
def send_states():
    ...
    states = dict(
        datasets = dict(
            count = len(weight_by_dataset_name),
            weights = sum(
                weight
                for weight in weight_by_dataset_name.itervalues()
    ...
    try:
        ...
        log.error(u'An error occured while setting states: {}'.format(states))
